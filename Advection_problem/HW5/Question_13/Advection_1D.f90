...
! Declare inital value
  ...
  N_step = T_end / dt
  b2 = 0.d0
  b3 = (-aa * dx**2.d0)/6.0 * (1-CFL**2.d0)
  b4 = (-aa * dx**3.d0)/8.0 * (1-CFL**2.d0)*CFL
  A1 = 1.d0
  A2 = 0.6d0
  k1 = 1.d0
  k2 = 3.d0
  sigma_1 = b4 * (k1*pi)**4.d0
  sigma_2 = b4 * (k2*pi)**4.d0
  beta_1  = k1*pi*aa - k1*pi - b3 * (k1*pi)**3.d0
  beta_2  = k2*pi*aa - k2*pi - b3 * (k2*pi)**3.d0
!-----------------------------------------------------------
! Print the variables
...
! Find the modified analytical solution u_M()
    do jj = 0, N+1
      u_M(jj) = A1 * dexp(sigma_1 * tt) * dsin(k1*pi*(x_coor(jj) - aa*tt) + beta_1*tt) &
              + A2 * dexp(sigma_2 * tt) * dsin(k2*pi*(x_coor(jj) - aa*tt) + beta_2*tt) 
    enddo
