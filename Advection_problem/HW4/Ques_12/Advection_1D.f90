...
! Declare inital value
  ...
  dt = CFL * dx / aa
  pi = 4.d0*atan(1.d0)
  T_end = 10.0 * L / aa                 ! I don't know how to determine the end time
  N_step = T_end / dt
  b2 = aa*dx/2.d0 * (1-CFL)
  ...
  k1 = 1.d0
  k2 = 3.d0
  sigma_1 = -b2 * (k1*pi)**2.d0 + b4 * (k1*pi)**2.d0
  sigma_2 = -b2 * (k2*pi)**2.d0 + b4 * (k2*pi)**2.d0
  beta_1  = -b3 * (k1*pi)**3.d0
  beta_2  = -b3 * (k2*pi)**3.d0
!-----------------------------------------------------------
! Print the variables
...
! tt = 0
  ...
  enddo
  do jj = 0, N+1
    u_old(jj) = A1 * dsin(k1*pi * x_coor(jj)) + A2 * dsin(k2*pi * x_coor(jj))
...
!-----------------------------------------
! Apply periodic B.C.
  u_old(0)   = u_old(N) ! u_old(N) is new too
  u_old(N+1) = u_old(1) ! u_old(1)  is the new time step vaule
...
! Find the modified analytical solution u_M()
    do jj = 0, N+1
      u_M(jj) = dexp(sigma_1 * tt) * A1 * dsin(k1*pi*(x_coor(jj) - aa*tt) + beta_1*tt) &
              + dexp(sigma_2 * tt) * A2 * dsin(k2*pi*(x_coor(jj) - aa*tt) + beta_2*tt) 
    enddo
...
100 format(A9, f14.8)
110 format(A9, I6)
120 format(f14.8,2x, I5, f14.8)
130 format(f14.8,2x, I5, f14.8, f14.8)
140 format(f14.8,2x, I5, 4f14.8)
!-----------------------------------------------------------
! Variables
! N                     = grid resolution
! dx                    = space step
