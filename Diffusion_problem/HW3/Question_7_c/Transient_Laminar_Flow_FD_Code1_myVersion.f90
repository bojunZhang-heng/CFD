...
!  CFL = dt * nu / dy**2.d0
  ...
  open(10, file='round-off_1.txt', status='unknown')
  open(20, file='round-off_2.txt', status='unknown')
  open(30, file='round-off_3.txt', status='unknown')
  open(40, file='round-off_4.txt', status='unknown')
    ...
    do jj = 1, N+1
      ...
      if(jj == y2) then
        ...
        if(t_dimensionless .lt. 5.0) then
          write(file(cc), 100)  ycc, t_dimensionless, abs(u(jj)-u_theory_m) / u0              
          write(*, 100)         ycc, t_dimensionless, abs(u(jj)-u_theory_m) / u0              
        endif
      endif
    enddo
  enddo
