...
!
      ...
      do jj = 1, n2
        do ii = 1, n1
          ...
          duy = ((qx(ii, jj+2)  - qx(ii, jj)) / 2.0 / dy &
              + (qx(ii+1, jj+2) - qx(ii+1, jj)) /2.0 / dy) / 2.0
          dvy = ((qy(ii+2, jj)  - qy(ii, jj)) / 2.0 / dx &
              + (qy(ii+2, jj+1) - qy(ii, jj+1)) / 2.0 / dx) / 2.0
          alam(ii, jj) = 0.0
          s9 = dux*dvy - duy*dvx
...
! Rescale to match the LBM code
        write(13, *) strmfn(ii, jj) * 96.0 / 10.0
        enddo
!       write(13,906)
      enddo
!
