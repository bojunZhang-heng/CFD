...
!
      ...
      do jj = 1, n2p2
        do ii = 1, n1p1
          sum = sum + (qx(ii, jj) - qxold(ii, jj))**2.d0
          sumtot = sumtot + (qx(ii, jj))**2.d0
        enddo
      enddo
      epsmax = sqrt(sum/sumtot)
! write steady state parameter to file
      if (mod(itime, 50) .eq. 0.d0) write(100,*) tau, epsmax

      if (mod(itime, 10) .eq. 0.d0) write(*,*) itime, tau, epsmax
